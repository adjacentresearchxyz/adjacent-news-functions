...
async function handleScheduled(env) {
	...
	// load into r2 raw data 

	// receive a r2 event notification when the data is loaded and then send it into the queue
...
async function extractData(env): Promise<any> {
	...
	// const kalshiMarkets = await kalshi.processData()

	// allMarkets.push(...kalshiMarkets)

	// polymarket
	const polymarket = new PolymarketClient()
	const polymarketMarkets = await polymarket.processData()
