...
class KalshiClient:
    ...
    def __init__(
        ...
        self.http_utils = AsyncHTTPUtils(host)
    ...
    async def post(self, path: str, body: dict) -> Any:
        ...
        response = await self.http_utils.post(
            path, data=body, headers=self.request_headers()
        )
    ...
    async def get(self, path: str, params: Dict[str, Any] = {}) -> Any:
        ...
        Returns the response body. Raises an HttpError on non-2XX results."""
        # await self.rate_limit()
        response = await self.http_utils.get(
            path, headers=self.request_headers(), params=params
        )
        # self.raise_if_bad_response(response)
        return response
    ...
    async def delete(self, path: str, params: Dict[str, Any] = {}) -> Any:
        ...
        response = await self.http_utils.delete(
            path, headers=self.request_headers(), params=params
        )
        self.raise_if_bad_response(response)
    ...
    def raise_if_bad_response(self, response) -> None:
        if response.status_code not in range(200, 299):
            raise HttpError(response.reason_phrase, response.status_code)
    
    def query_generation(self, params: dict) -> str:
        # Exclude 'self' key from the parameters
        relevant_params = {k: v for k, v in params.items() if v is not None and k != 'self'}
        if relevant_params:
...
class ExchangeClient(KalshiClient):
    def __init__(self, exchange_api_base: str, email: str, password: str):
        super().__init__(exchange_api_base, email, password)
        ...
        # Initialize token and user_id as None, will be set in async_init
        self.token = None
        self.user_id = None

    async def async_init(self):
        login_json = json.dumps({"email": self.email, "password": self.password})
        result = await self.post(path="/login", body=login_json)
